...
// Función auxiliar para obtener el BR máximo
...
// Función para mostrar éxito al guardar
...
// Función para mostrar error al guardar
        ...
        html: `
            <div style="text-align: left;">
                ...
                <ul>
                    ...
                    <li>Sesión de paciente no válida</li>
                </ul>
...
// Función para interpolar valores (ya existente)
function interpolateValue(score, scaleId) {
    const table = conversionTable[scaleId];
    if (!table) {
        console.error(`Tabla de conversión no encontrada para la escala ${scaleId}`);
...
// Función para obtener categoría según puntuación BR (ya existente)
function getCategoria(br) {
    if (br >= 85 && br <= 115) return 'Indicador Elevado';
    if (br >= 75 && br <= 84) return 'Indicador Prominente';
    if (br >= 60 && br <= 74) return 'Indicador Presente';
...
// Función para validar respuestas (ya existente)
function validateResponses() {
    const validationMessage = document.getElementById('validation-message');
    const calculateBtn = document.getElementById('calculate-btn');
